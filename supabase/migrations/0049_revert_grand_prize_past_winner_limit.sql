-- Revert 0048 — restore the past-winner limitation on the grand prize.
-- Per organizer: the grand prize (14K gold ring) should again EXCLUDE
-- tickets that already won another prize. So the grand draws from PAID
-- entries (is_complimentary = false) that have NOT already won.
...
begin
  ...
  if p_prize_id = 'prize_grand' then
    -- Grand prize: PAID entries only AND exclude already-won tickets.
